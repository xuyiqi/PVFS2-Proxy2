...
for ($it=1; $it<=$Rings; $it++)
{
	...
	for ($rank=1; $rank<=$Ranks; $rank++)
	{
		...
		while ($record = <CHECKBOOK>) {
			...
			else 
			{
				...
				if ($record_value<$startt)
				{
				$prev_value=$record_value;

					next;
				}
				elsif ($record_value>$endt)
				{
					last;
				}
				else
				{
					...
					$new_interval=$record_value-$prev_value;
					print RINGFILE "$new_interval,";
					$total_requests++;
	$prev_value=$record_value;
	...
	print RINGFILE "total:$total_requests\n";
	close (RINGFILE); 
