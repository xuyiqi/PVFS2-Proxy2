  ...
  while (1) {
	...
	for (i = 0; i < s_pool.pool_size; i++)	//iterate through the whole socket pool to set specific listening s
	{
	  if (s_pool.socket_state_list[i].mode != ACCEPT) {
		if ((s_pool.socket_state_list[i].buffer_size != 0 && s_pool.socket_state_list[i].buffer_tail == s_pool.socket_state_list[i].buffer_size)
	  ...
	  if (s_pool.socket_state_list[i].mode == WRITE) {
		...
		if (s_pool.socket_state_list[counter_index].buffer_tail - s_pool.socket_state_list[counter_index].buffer_head == 0)
	  ...
	  for (i = 0; i < s_pool.pool_size; i++)	//iterate through the whole socket pool for returned events
	  {
		...
		if (s_pool.poll_list[i].revents & POLLIN) {
		  ...
		  else					//this is a real read event of data from a socket
		  {
			...
			else if (s_pool.socket_state_list[i].source == CLIENT && s_pool.socket_state_list[i].completed_size == 0) {
			...
			if (s_pool.socket_state_list[i].locked == 0) {
			  ...
			  if (s_pool.socket_state_list[i].completed_size == s_pool.socket_state_list[i].job_size && s_pool.socket_state_list[i].job_size > 0) {
				...
				if (scheduler_on && s_pool.socket_state_list[i].source == CLIENT && s_pool.socket_state_list[i].op == PVFS_DATA_FLOW)	// a PVFS_WRITE
				{
 ...
 * */
				...
				if (scheduler_on && s_pool.socket_state_list[i].source == SERVER && s_pool.socket_state_list[i].op == PVFS_DATA_FLOW)	// a PVFS_READ
				{
		...
		if (s_pool.poll_list[i].revents & POLLOUT)	//the port is ready to write without blocking
		{
			...
			)					//this is to avoid sending partial get_config buffer without modifying it.
		  {
			...
			if (sent_size == 0 || sent_size == -1) {
			  ...
			  if (ready_size > sent_size) {	//if we only sent partial data, then we need the left data to be in the beginning part of the buffer
