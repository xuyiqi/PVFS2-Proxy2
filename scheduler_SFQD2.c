...
int sfqd2_current_size(int sock_index, long long actual_data_file_size)
{
...
/*		struct heap_node * hn  = heap_peek(sfqd2_packet_cmp, sfqd2_heap_queue);
		next  = heap_node_value(hn);
		struct sfqd2_queue_item *next_item = (struct sfqd2_queue_item *)(next->embedded_queue_item);
		long long diff=app_stats[i].app_throughput - total_throughput * app_stats[i].app_weight/ total_weight;
		if ( diff > 10240)
		{
...
*/
		...
		if (i>=s_pool.pool_size)
		{
			...
			heap_take(sfqd2_packet_cmp, sfqd2_heap_queue);
			goto next_retry;
		}
/*		char bptr[20];
	    struct timeval tv;
