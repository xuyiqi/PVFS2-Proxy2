 ...
 *
 *      Adapted from http://
 *      only add_remove (extract_min) is used
 *      count implementation modified a little bit to count for each application/total as well
 *      decrease/increase key will probably be used later when the keys needs to be updated after insertion.
 */
#include "heap.h"
#include <stdlib.h>
#include <limits.h>
...
void heap_init(struct heap* heap)
{
	heap->head = NULL;
	heap->min  = NULL;
	heap->count=(int*)malloc(num_apps*sizeof(int));
	memset(heap->count, 0, num_apps*sizeof(int));
	heap->all_count=0;
}
...
void __heap_link(struct heap_node* root,
...
/* merge root lists */
struct heap_node* __heap_merge(struct heap_node* a,
					     struct heap_node* b)
{
...
struct heap_node* __heap_reverse(struct heap_node* h)
{
...
void __heap_min(heap_prio_t higher_prio, struct heap* heap,
...
void __heap_union(heap_prio_t higher_prio, struct heap* heap,
				struct heap_node* h2)
{
	struct heap_node* h1;
...
struct heap_node* __heap_extract_min(heap_prio_t higher_prio,
						   struct heap* heap)
{
			       ...
			       struct heap_node* node)
{
	...
	//fprintf(stderr,"heap count before insertion:%i\n",heap->count);

	struct generic_queue_item * it = heap_node_value(node);
	int index = ((struct sfqd_queue_item *)(it->embedded_queue_item))->app_index;
	heap->count[index]++;
	heap->all_count++;
	//fprintf(stderr,"heap count after insertion:%i\n",heap->all_count);
}



void __uncache_min(heap_prio_t higher_prio, struct heap* heap)
{
	struct heap_node* min;
	if (heap->min) {
		min = heap->min;
					  ...
					  struct heap* heap)
{
	...
	//fprintf(stderr,"heap count before removal:%i\n",heap->count);
	struct generic_queue_item * it = heap_node_value(node);
	int index = ((struct sfqd_queue_item *)(it->embedded_queue_item))->app_index;
	heap->count[index]--;
	heap->all_count--;
	//fprintf(stderr,"heap count after removal:%i\n",heap->all_count);
	return node;
}
