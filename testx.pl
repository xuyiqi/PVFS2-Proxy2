...
print "MDTEST:\t\t\t$mdlocation\n";
print "IOZONE:\t\t\t$iozonelocation\n";
print "METARATES:\t\t\t$metarateslocation\n";
print "MPIIOTEST:\t\t\t$mpiiotestlocation\n";
...
for ($it=1; $it<=$Iterations; $it++)
{
	...
	for ($l=$NumApp; $l>0; $l--)
	{
		...
		for ($i=0; $i<$#clts+1;$i++){
			...
			$r=`$result`;
			print $r;
			$r = "sudo ssh $ClientNodes[$clts[$i]-1] /home/users/yiqi/drop_caches";
			print "$r\n";
			$result = `$r`;
			print $result;
	...
	if ($DropCaches[$it-1]==1)
	{
		...
		for (my $dci=0; $dci<$#AllServerNodes+1; $dci++){
			$r = "sudo ssh $AllServerNodes[$dci] /home/users/yiqi/drop_caches";
			print "$r\n";
			$result = `$r`;
			print $result;
	...
	for ( my $count = 0; $count < $#AllServerNodes+1; $count++) {
		...
		if ($mpstat ==1)
		{
	        ...
	        if ($Algorithm eq 'NATIVE'){
        	        print "######### Native Mode Skipping proxy start on $AllServerNodes[$count] .... #########\n";
		}
		else
		{
        		my $pid = fork();
	...
	else
	{
                ...
                for (my $l=0; $l<$NumApp; $l++)
                {
                        ...
                        for ($i=0; $i<$#clts+1;$i++)
                        {
                                ...
                                print BOLD, RED, "$r\n", RESET;
                                $result = `$r`;
                                print BOLD, RED, $result, RESET;
				$tresult = $result;
				$tresult =~ s/^\s+|\s+$//g;
				if ($tresult eq '' || index($result, $MetaServerNodes[0]) == -1 || index($result, "pvfs2") == -1 )
				{
					print BOLD, RED, "client node $ClientNodes[$clts[$i]-1] is not mounting PVFS2 correctly, check the proxy output or server log.\n", RESET;
	...
	for ( my $count = 0; $count < $#AllServerNodes+1; $count++) {
	        ...
	        @grep1=split(' ', $result);
	        print BOLD, BLUE, "pvfs2: $grep1[0]\n", RESET;

		$grep1[0] =~ s/^\s+|\s+$//g;

		if ($grep1[0] eq '')
		{
			print BOLD, RED, "pvfs2 does not work on AllServerNodes[$count]\n", RESET;
			exit 5;
		}
	        ...
	        #split
                ...
                if ($grep2[0] eq '' && !$Algorithm eq 'NATIVE')
                {
                        print BOLD, RED, "proxy2 does not work on AllServerNodes[$count]\n", RESET;
                        exit 5;
                }


        	@grep2=split(' ', $result);
	        print BOLD, RED, "proxy2: $grep2[0]\n", RESET;
		if ($grep2[0] eq '')
		{
			$grep2[0]='1';
		}
		...
		{		
                ...
                $r = "ssh $ServerNodes[$count] taskset -pc 1 $grep1[0]";
                print "$r\n";
                $result = `$r`;
                print $result;
                ...
                if ($grep2[0] != 1)
                {
                        $r = "ssh $ServerNodes[$count] taskset -pc 2 $grep2[0]";
	                print "$r\n";
	       	        $result = `$r`;
                	print $result;
                }
	...
	if ($TestTime eq "WAIT")
	{
			...
			my $childs=$#childs_mpi+1;
        	        ...
        	        foreach (@childs_mpi) {
                	        my $tmp = waitpid($_, 0);
                        	print "wait done with mpi pid $tmp\n";
                	}
			print "killing sleep thread - init is $Initialize\n";
			system ("kill $sleepid");
			system ("kill $sleepid2");
		        if ($Initialize==1)
        		{
	...
	elsif ($TestTime eq 'WAITFASTEST')
	{
			...
			exit 0;
                ...
                my $tmp = waitpid($childs_mpi[0], 0);
                print "fastest done with mpi pid $tmp\n";
	...
	else
	{
		...
		print "timer done with child process for MPIs...killing the ring\n";
		#Use form() again
		for ( my $count = 1; $count <= $NumApp; $count++) {
			my $pid = fork();
			if ($pid) {
...
print "=======================End of test program [v10.0]======================\n\n";
...
sub sub_kill_mpi {
	...
	$r = "ssh $node killall -w IOR multi-md-test metarates";
	print "$r\n";
	$result = `$r`;
	print $result;		
}
...
sub import_app_params {
	...
	print "npblocation is $npblocation\n";
        ...
        $record=<CHECKBOOK>;
        $iozonelocation=split_value('=', $record, 1);
        print "iozonelocation is $iozonelocation\n";

        $record=<CHECKBOOK>;
        $metarateslocation=split_value('=', $record, 1);
        print "metarateslocation is $metarateslocation\n";

        $record=<CHECKBOOK>;
        $mpiiotestlocation=split_value('=', $record, 1);
        print "mpiiotestlocation is $mpiiotestlocation\n";
...
sub exec_benchmark{
    ...
    if ($pid) {
		...
		elsif ($bench eq 'iozone')
		{
			exec_iozone($num, $wait);			
		}
		elsif ($bench eq 'metarates')
		{
			exec_metarates($num, $wait);
		}
		elsif ($bench eq 'mpiiotest')
		{
			exec_mpiiotest($num, $wait);
		}
		elsif ($bench eq 'virtual')
		{
			exec_virtual();
		}
		else
...
sub exec_virtual{
	...
	{
		our $sleepid2 = $pid;
		print  " !!!!!!!!!!!!!!!! WAITING ON VIRUTAL PID $sleepid2 !!!!!!!!!!!!!!!\n";
		waitpid($pid,0);
	}
	else
	{
		print " *************** executing VRITUAL ****************\n";
		sleep(7200);
	}
}
sub exec_MD{
...
sub sub_cp{
	...
	$r = "ssh $nodename cp /data/yiqi.cp.datafile $pvfsmnt/datafile.r$ring.p$proc";
	print "$r\n";
	$result = `$r`;
	print $result;
        $r = "ssh $nodename cp /data/yiqi.cp.datafile $pvfsmnt/datafile.r$ring.p$proc";
        ...
        $result = `$r`;
        print $result;
        $r = "ssh $nodename cp $pvfsmnt/datafile.r$ring.p$proc $pvfsmnt/datafile.r$ring.p$proc.2";
        print "$r\n";
        $result = `$r`;
        print $result;
...
sub exec_iozone{
        ...
        my $num=shift; #app number
	# always execute in simulated mode
	# record size, file size, file name(s)
	# auto mode, test item (-i 0,1,.etc.)
	# do not unlink file (-w)
	# throughput mode (-t # -T)
	# output file
        my @clts=split(',',$app_clts[$num-1]);
        
	#fork into each remote machine
	#in the child, run each individual iozone command
	my $appmachine = $ClientNodes[$clts[0]-1];
        ...
        for ($i=0; $i<$#clts+1;$i++)
        {
                ...
                for ($j=0; $j< $#clts+1; $j++)
                {
                        ...
                        if ($pid) {
                                # parent
                                print "pid is $pid, parent $$\n";
                                push(@childs_iozone, $pid);
                        } elsif ($pid == 0) {
                                # child
                                sub_iozone($ClientNodes[$clts[$i]-1], $num, $npprocesses);
                                exit 0;
                        } else {
                                die "couldnt fork: $!\n";
                        }
        ...
        if ($Initialize==1)
        {
                print "########## [iozone] Initialize only tag found, exiting before anything else happens... ###########\n";
                return;
        }
...
sub sub_iozone{
	...
	$appmachine=shift;$ring=shift;$np=shift;
	$r = "ssh $appmachine ";
	$r = $r . "$iozonelocation/iozone ";
	if ($np > 0)
	{
        	$r = $r."-a -t $np -T -F";
	}
	...
	$r = $r.$pvfsmnt."/"."$app_fname[$num-1] $app_rw[$num-1] -r $app_block[$num-1] -s $app_fsize[$num-1]";
	$r = $r ." ".$app_other_param[$num-1];
	print "$r\n";
}

sub exec_metarates{
        print "%%%%%%%%%%%%%% Running METARATES... %%%%%%%%%%%%%%%\n";
   ...
   #     }
        ...
        $r = "ssh $ClientNodes[$clts[$i]-1] mkdir $changedfns";
        print BOLD, RED, "$r\n", RESET;
        $result=`$r`;
        print BOLD, RED, $result, RESET;

        $r = "ssh $ClientNodes[$clts[$i]-1] stat $changedfns";
        print BOLD, RED, "$r\n", RESET;
        $result=`$r`;
        print BOLD, RED, $result, RESET;

        $r="$pvfsutilitylocation/pvfs2-mkdir $changedfns";
        print BOLD, RED, "$r\n", RESET;
        $result=`$r`;
        print BOLD, RED, $result, RESET;
        ...
        $r="$pvfsutilitylocation/pvfs2-stat $changedfns";
        print BOLD, RED, "$r\n", RESET;
        $result=`$r`;
        print BOLD, RED, $result, RESET;
	...
	sleep(50);
        ...
        #./multi-md-test -d /home/users/yiqi/mnt/mdtest18 -i -n 800 -s 65536 -a 1 -p 1 -c 48 -i
	#./metarates -d dirname -k keep -n numfiles -u unique folders -U utime -C create time -S stat time -s fsync call
        $r = "ssh $appmachine $mpilocation/mpiexec -machinefile $softwarelocation/$app_machine[$num-1] -n $app_client[$num-1] $metarateslocation/metarates";
        $r = $r." -d ".$changedfns." -n ".$app_block[$num-1];
        $r = $r." ".$app_other_param[$num-1];
        print "$r\n";
        if ($Initialize==1)
        {
                print "########## [MDTEST] Initialize only tag found, exiting before anything else happens... ###########\n";
                exit(0);
        }
        $result = `$r`;
        print $result;
