...
FILE * 	pFile_log_time_stamp;
FILE *  pTrace;

char 	log_file_name[256];
char	log_sentence[1000];
...
int bufferCount=0;
int bufferMax=10;
//struct TraceLine TraceBuffer[bufferMax];
int inTrace = 0;
int outTrace = 0;
     ...
     char ** argv)
{
    ...
    //sprintf(log_sentence,"time\toffset\toperation\n");
    //fputs(log_sentence,pFile_log_time_stamp);
...
/**
* Generate a random operation access WRITE or READ
**/
int get_random_op(int write_prob)
{
        int ret = -1;
        int random;
            ...
            int           access)
{
    ...
    //setup timer
    struct timeval last_update,first_update, diff_last, diff_first;
    gettimeofday(&first_update, 0);
    ...
    if (initialTestParams.dataRate!=NULL)
    {
    	stage_rate=initialTestParams.datarates[current_stage].ttl_data_rate*1048576/test->numTasks;
    	fprintf(stderr,"initial rate: %f\n", stage_rate);
    }

    double start_time =  MPI_Wtime();
    		//(double)first_update.tv_sec+(double)first_update.tv_usec/1000000.0f;// =  MPI_Wtime();
	struct timeval current_time;
	int salt_sleep=0;
	int us_to_sleep = -1;
	int last_salt_sleep = 0;
	int last_completion_time=2000000;//in usec, 2 secs
	...
	// Generate random operation if allowed
		if(test->randomOp)
		{
			access = get_random_op(test->randomOp_wp);
			if (rank == 0 && verbose >= VERBOSE_1) {
				if (access == 0)
					printf ("WRITE\n");
				else if (access == 2)
		...
		if (us_to_sleep!=-1)
		{
			salt_sleep = w*us_to_sleep/test->numTasks;//n ms*#rank
			if (salt_sleep > last_salt_sleep+us_to_sleep)
			{
				fprintf(stderr,"%i: salt sleep adjust from %i ", rank, salt_sleep);
				salt_sleep = salt_sleep %  (last_salt_sleep+us_to_sleep);
				fprintf(stderr,"to %i because it was > %i (last_salt %i + after_sleep %i)\n",
						salt_sleep, last_salt_sleep+us_to_sleep, last_salt_sleep, us_to_sleep);
			}
			//this salt sleep cannot be larger than the left over time of last time!

			last_salt_sleep = salt_sleep;

			fprintf(stderr,"%i: salt_sleeping %i seconds\n",rank, salt_sleep);
			usleep(salt_sleep);
		}
		else//even though last time we were in a hurry, we don't want to swarm requests either...
		{
			salt_sleep = w*last_completion_time/test->numTasks;//n ms*#rank
			fprintf(stderr,"%i: skipping salt sleep because last I/O is lagging behind\n", rank);
		}


		gettimeofday(&last_update, 0);
		var_time =  MPI_Wtime();
				//(double)last_update.tv_sec+(double)last_update.tv_usec/1000000.0f;// =  MPI_Wtime();


		sprintf(log_sentence,"%f\t%ld\t%d\t%f",var_time,test->offset,access,var_time-start_time);
		fputs(log_sentence,pFile_log_time_stamp);
		fflush(pFile_log_time_stamp);
		...
		var_time =  MPI_Wtime();
				//(double)current_time.tv_sec+(double)current_time.tv_usec/1000000.0f;// =  MPI_Wtime();
		sprintf(log_sentence,"\t finished at %f\n",var_time);
		fputs(log_sentence, pFile_log_time_stamp);
		fflush(pFile_log_time_stamp);

		timersub(&current_time, &last_update, &diff_last);
		timersub(&current_time, &first_update, &diff_first);
		last_completion_time = diff_last.tv_sec*1000000+diff_last.tv_usec;
		if (initialTestParams.dataRate!=NULL)
		{


			float seconds= diff_last.tv_sec+diff_last.tv_usec/1000000.0f;
			float actual_rate= amtXferred/seconds;
			fprintf(stderr,"elapsed time:%f, actual rate %f/%f\n", seconds, actual_rate,stage_rate);



			//which stage am i in?
			int desired_bytes= stage_rate*seconds;
			int surplus=MAX(0,amtXferred-desired_bytes);

			if (surplus>0)
			{
				//calculate surplus
				fprintf(stderr,"over provisioning\n");
				us_to_sleep = amtXferred * 1000000 / stage_rate - diff_last.tv_sec*1000000 -diff_last.tv_usec;
				fprintf(stderr,"%i * 1000,000 / %f - %i - %i\n",
						amtXferred, stage_rate, diff_last.tv_sec*1000000 - diff_last.tv_usec);
				fprintf(stderr,"sleeping %i microseconds -  %i.%06i seconds\n",
						us_to_sleep, us_to_sleep/1000000, us_to_sleep-us_to_sleep/1000000);
				usleep(us_to_sleep);
		...
		dataMoved += amtXferred;
			fprintf(stderr,"rank: %d, completed: %d\n",rank , amtXferred);
			//get time difference
			//get current rate
			//if higher, sleep so data_transferred/(current_time+sleep_time)=desired_rate
			//desired_rate=ttl_ranks/number_of_ranks
		...
		if (initialTestParams.dataRate!=NULL)
		{
			...
			if (diff_first.tv_sec>=initialTestParams.datarates[current_stage+1].timestamp_begin)
			{
				current_stage++;
				fprintf(stderr,"%i seconds have passed, new stage begins...desired rate is %i\n",
						diff_first.tv_sec, initialTestParams.datarates[current_stage].ttl_data_rate);
				stage_rate=(float)initialTestParams.datarates[current_stage].ttl_data_rate * 1048576.0 / (float)test->numTasks;
				fprintf(stderr,"stage rate per node is %f\n", stage_rate);
				if (stage_rate<0.1)
				{
					int sectosleep = initialTestParams.datarates[current_stage+1].timestamp_begin-diff_first.tv_sec;
					fprintf(stderr,"next stage is at %i sec, sleeping %i seconds\n",
							initialTestParams.datarates[current_stage+1].timestamp_begin, sectosleep);
					sleep(sectosleep);
					current_stage++;
					stage_rate=(float)initialTestParams.datarates[current_stage].ttl_data_rate * 1048576.0 / (float)test->numTasks;
					fprintf(stderr,"NEW stage begins...desired rate is %i\n",
								initialTestParams.datarates[current_stage].ttl_data_rate);
...
int fill_trace(FILE * from, struct TraceLine buffer)
{
	//traceIn, traceOut;
...
*/
	...
	{
		while (bufferCount<bufferMax && fscanf(from, "%f\t%i\t%l\t%i\t%i\t%i",
				&time, &file_id, &offset, &size, &rw, &appid)>0)
		{
			//generate new value
			//fill fieldsw
			//TraceBuffer[inTrace]= NULL;//new value
			inTrace= (inTrace+1) % bufferMax;
		}
	}
	else{
		return 0;
